...
public class MyUnityEditorTool : MonoBehaviour
{
    ...
    /*
    //加密配置文件
    [MenuItem("Tools/文件加密解密/加密所有配置文件")]
    public static void EncryptAllConfigFiles()
    {
        string dir = MyFileUtil.CacheDir;
        List<FileUnit> fileList = new List<FileUnit>();
        MyFileUtil.GetRelativeFileListWithSpecialFileType(dir, "", ref fileList, ".xml");
        MyFileUtil.GetRelativeFileListWithSpecialFileType(dir, "", ref fileList, ".csv");
        MyFileUtil.GetRelativeFileListWithSpecialFileType(dir, "", ref fileList, ".txt");

        for (int i = 0; i < fileList.Count; ++i)
        {
            FileUnit unit = fileList[i];
            //EncryptDecryptUtil.EncryptFile(unit.fullPath);
            EncryptFileImp(unit.fullPath);
        }

        AssetDatabase.Refresh();
        Debug.Log("MyToolUtil.EncryptAllConfigFiles:文件加密结束");
    }

    //解密配置文件
    [MenuItem("Tools/文件加密解密/解密所有配置文件")]
    public static void DecryptAllConfigFiles()
    {
        string dir = MyFileUtil.CacheDir;
        List<FileUnit> fileList = new List<FileUnit>();
        MyFileUtil.GetRelativeFileListWithSpecialFileType(dir, "", ref fileList, ".xml");
        ...
        {
            FileUnit unit = fileList[i];
            //EncryptDecryptUtil.DecryptFile(unit.fullPath);
            DecryptFileImp(unit.fullPath);
        }

        AssetDatabase.Refresh();
        Debug.Log("MyToolUtil.DecryptAllConfigFiles:文件解密结束");
    }

    //加密文件
    [MenuItem("Tools/文件加密解密/加密文件")]
    static void EncryptFile()
    {
        if (Selection.activeInstanceID != 0)
        {
            ...
            filePath += path;
            //EncryptDecryptUtil.EncryptFile(filePath);
            EncryptFileImp(filePath);
            AssetDatabase.Refresh();
            Debug.Log("MyTools.EncryptFile:文件加密完成");
        }
    }

    //解密文件
    [MenuItem("Tools/文件加密解密/解密文件")]
    static void DecryptFile()
    {
        if (Selection.activeInstanceID != 0)
        {
            ...
            string filePath = Application.dataPath.Replace("Assets", "");
            filePath += path;
            //EncryptDecryptUtil.DecryptFile(filePath);
            DecryptFileImp(filePath);
            AssetDatabase.Refresh();
            Debug.Log("MyTools.DecryptFile:文件解密完成");
        }
    }

    static string EncryptInfoFileName
    {
        get
        {
            //return Application.persistentDataPath + "/Editor_EncryptInfo.csv";
            return Application.dataPath + "/Editor/Editor_EncryptInfo.csv";
        }
    }

    static Dictionary<string, bool> mFileEncryptInfoDict = new Dictionary<string, bool>();
    static Dictionary<string, bool> FileEncryptInfoDict
    {
        get
        {
            if (mIsInitFileEncryptInfoDict == false)
            {
                if (File.Exists(EncryptInfoFileName))
                {
                    //读取配置文件
                    CSVParser csv = new CSVParser();
                    csv.ParseFile(EncryptInfoFileName);
                    for (int i = 2; i < csv.GetRowSize(); ++i)
                    {
                        string name = csv.GetString(i, 0);
                        string str = csv.GetString(i, 1).ToLower();

                        if (str == "true")
                        {
                            mFileEncryptInfoDict.Add(name, true);
                        }
                        else
                        {
                            mFileEncryptInfoDict.Add(name, false);
                        }
                    }
                }

                mIsInitFileEncryptInfoDict = true;
            }
            return mFileEncryptInfoDict;
        }
    }
    static bool mIsInitFileEncryptInfoDict = false;

    //设置文件加密状态
    public static void SetFileEncryptState(string filePath, bool isEncrypt)
    {
        if (FileEncryptInfoDict.ContainsKey(filePath))
        {
            FileEncryptInfoDict[filePath] = isEncrypt;
        }
        else
        {
            FileEncryptInfoDict.Add(filePath, isEncrypt);
        }

        //写配置文件
        StreamWriter writer = new StreamWriter(EncryptInfoFileName);
        writer.WriteLine("文件名,是否加密");
        writer.WriteLine("string,string");
        foreach (string key in FileEncryptInfoDict.Keys)
        {
            string str = string.Format("{0:s},{1:s}", key, FileEncryptInfoDict[key]);
            writer.WriteLine(str);
        }
        writer.Flush();
        writer.Close();
    ...
    //加密配置文件
    static void EncryptFileImp(string filePath)
    {
        if (FileEncryptInfoDict.ContainsKey(filePath))
        {
            bool isEncrypt = FileEncryptInfoDict[filePath];
            if (isEncrypt)
            {
                return;
            }
            ...
            EncryptDecryptUtil.EncryptFile(filePath);
            SetFileEncryptState(filePath, true);
        }
        else
        {
            EncryptDecryptUtil.EncryptFile(filePath);
    ...
    //解密配置文件
    static void DecryptFileImp(string filePath)
    {
        if (FileEncryptInfoDict.ContainsKey(filePath))
        {
            bool isEncrypt = FileEncryptInfoDict[filePath];
            ...
            EncryptDecryptUtil.DecryptFile(filePath);
            SetFileEncryptState(filePath, false);
        }
    }

    //
    [MenuItem("Tools/文件加密解密/清理编辑器内存(清除内存中记录的配置文件加密解密状态)")]
    static void ClearEncryptDecryptMemoryLog()
    {
        FileEncryptInfoDict.Clear();
    }

    //加密文件
    [MenuItem("Tools/文件加密解密/强制加密文件(不考虑重复加密，选择一次加密一次)")]
    static void ForceEncryptFile()
    {
        if (Selection.activeInstanceID != 0)
        {
            string path = AssetDatabase.GetAssetPath(Selection.activeInstanceID);
    ...
    //解密文件
    [MenuItem("Tools/文件加密解密/强制解密文件(不考虑重复解密，选择一次解密一次)")]
    static void ForceDecryptFile()
    {
        if (Selection.activeInstanceID != 0)
        {
            int id = Selection.activeInstanceID;
    ...
    public static bool IsFileEncrypted(string filePath)
    {
        if (FileEncryptInfoDict.ContainsKey(filePath))
        {
            return FileEncryptInfoDict[filePath];
        }
